...
export class LocalLoginComponent implements OnInit {
    ...
    onLoginClick() {
        ...
        this.service.regist('/user/login', this.userOption).subscribe(async token => {
                this.model.userInfoDate.userId = token['userId'];
                this.model.userInfoDate.role = token['role'];
                localStorage.setItem(CommonConstance.LOCALSTORAGE.TOKEN, String(1));
                this.loginOptions.userId = token['userId'];
                ...
                if(this.model.userInfoDate.role == 0){
                    this.router.navigate(['/pages/teacherCourse']);
                } else{
                    this.router.navigate(['/pages/studentCourse']);
                }
            },
            error => {
