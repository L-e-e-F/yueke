...
export class RegisterComponent implements OnInit {
    ...
    async onSubmit() {
        ...
        this.userHttpService.regist('/user/regist', this.add).subscribe(
            async res => {
                ...
                localStorage.setItem(CommonConstance.LOCALSTORAGE.USER_ID, res['userId']);
                localStorage.setItem(CommonConstance.LOCALSTORAGE.USER_NAME, res['userName']);
    ...
    initForm() {
        this.myForm = this.fb.group(
            {
                userName: ['', Validators.compose([trimValidator, Validators.required])],
                role: ['', Validators.compose([trimValidator, Validators.required])],
